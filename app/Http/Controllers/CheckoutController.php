...
{
    /**
     * Open a Creem checkout for the plan the pilot picked.
     *
     * Answers JSON rather than an Inertia response: the pricing page stays
     * where it is and hands the URL straight to Creem's embed, so a pilot who
     * closes the overlay is back on the page they were reading rather than on a
    ...
    public function store(
        ...
        try {
            ...
            /*
             * Minting a checkout is a live API call, so this endpoint can fail
             * for reasons that have nothing to do with what was posted: an
             * unset API key, a product the account does not have, Creem being
             * down. None of those are a 500 the pilot can act on, and none of
             * them are theirs to read — the exception message names our
        ...
        if ($url === null) {
            /*
             * Reached by an unsold billing period, a sales-led tier, or a
             * product this environment has not configured. All three are "you cannot
